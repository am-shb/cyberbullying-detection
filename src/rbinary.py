...
#UnderSampling using Random under sampling approach
...
#The evaluation metrics used are f1_score and accuracy
  ...
  with open(base_dir('resultsb/r_{}_{}_{}.txt'.format(e,c,f)), 'w+') as out:
    out.write(','.join(l) + '\n')
...
for c, clf in classifiers.items():
  for e, emb in embeddings.items():
    for f, (train_index, test_index) in enumerate(kf.split(emb)):
      if os.path.exists(base_dir('resultsb/r_{}_{}_{}.txt'.format(e,c,f))):
        continue
